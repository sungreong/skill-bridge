...
로컬 워크스페이스의 AI 에이전트 스킬(Claude, Codex, Gemini, Cursor, Antigravity, `.agents` 등)을 중앙 Git 저장소와 연결해 관리하는 경량 데스크톱 도구 및 VSCode 익스텐션입니다.
...
- **검토** — `SKILL.md` 누락, 민감정보 의심 문자열, 스크립트, 깨진 링크, 워크스페이스 전용 경로 확인
- **Hydrate** — 재사용 가능한 개인 스킬 팩으로 프로젝트 초기 스킬 구성
...
- 데스크톱 자산 인벤토리에서 워크스페이스와 중앙 스킬 폴더의 경고, 파일 수, 변경 상태를 확인할 수 있습니다.
- 데스크톱 워크스페이스 관리에 워크스페이스별 자동 새로고침, 촘촘한 트리 컨트롤, 그룹 정리, 트리 글자 크기 조절이 추가되었습니다.
- VSCode 익스텐션에는 Personal Skill Home, 프로젝트 Hydrate, 개인 스킬 팩, Add/Move Wizard, 트리 필터, Transfer Manager 검토 프롬프트, Problems 뷰 품질 진단이 추가되었습니다.
- 기존 파일 덮어쓰기 흐름은 계속 diff 검토를 거치며, Skill Bridge는 자동 병합이나 사용자 의도 없는 자동 push를 하지 않습니다.
