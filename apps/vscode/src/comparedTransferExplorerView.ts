...
<head>
  ...
  <title>${localize("Compare and Apply Changes")}</title>
  <style>
...
<body>
  <div class="wrap sb-root">
    <div class="head sb-topbar">
      <div class="title">${localize("Compare and Apply Changes")}</div>
      <div class="controls">
        <div class="mode-switch" id="modeSwitch"></div>
        <input id="searchInput" placeholder="${localize("Search skills, agents, or groups...")}" />
        <button id="refreshBtn">${localize("Refresh")}</button>
      </div>
    </div>
    <div class="tabs" id="toolTabs"></div>
    <div id="summary" class="status-tabs"></div>
    <div id="resultMeta" class="result-meta"></div>
    <div class="hint">${localize("Unchanged items are kept collapsed, while changed skills and skill groups are reviewed by agent in the modified, new, and delete sections.")}</div>
  ...
  <script nonce="${nonce}">
    ...
    function statusInfoMap(){
      return {
        modified: { label: t("Modified"), cls: "s-modified", title: t("Modified"), desc: t("Items that exist on both sides but have different contents") },
        added: { label: t("New"), cls: "s-added", title: t("New"), desc: t("Items that only exist in the source and will be added to the target") },
        removed: { label: t("Delete"), cls: "s-removed", title: t("Delete"), desc: t("Items that only exist in the target and will be deleted when applied") },
        same: { label: t("Same"), cls: "s-same", title: t("Same"), desc: t("Items with identical contents on both sides") }
      };
    }
    ...
    function sourceLabel(){ return sourceSide() === "workspace" ? t("Workspace") : t("Central"); }
    function targetLabel(){ return targetSide() === "workspace" ? t("Workspace") : t("Central"); }
    function directionText(){ return sourceLabel() + " → " + targetLabel(); }
    function setStatus(message, tone){ const el = document.getElementById("statusLine"); el.textContent = message || t("Ready"); el.className = "status sb-status-bar " + (tone || "info"); }
    ...
    function renderPager(status, total){
      ...
      return '<div class="pager"><span>' + esc(from) + '-' + esc(to) + ' / ' + esc(total) + ' · ' + esc(PAGE_SIZE) + ' ' + esc(t("per page")) + '</span><div class="pager-actions">' +
        '<button class="ghost" data-action="page-section" data-status="' + esc(status) + '" data-dir="-1" ' + (page <= 1 ? "disabled" : "") + '>' + esc(t("Previous")) + '</button>' +
        '<span>' + esc(page) + ' / ' + esc(pages) + '</span>' +
        '<button class="ghost" data-action="page-section" data-status="' + esc(status) + '" data-dir="1" ' + (page >= pages ? "disabled" : "") + '>' + esc(t("Next")) + '</button>' +
        '</div></div>';
    }
    ...
    function groupChips(skill){
      ...
      if (w.length) chips.push(t("W group ") + w.slice(0,2).join(", ") + (w.length > 2 ? " +" + (w.length - 2) : ""));
      if (c.length) chips.push(t("C group ") + c.slice(0,2).join(", ") + (c.length > 2 ? " +" + (c.length - 2) : ""));
    ...
    function renderSummary(buckets){
      document.getElementById("summary").innerHTML = STATUS_ORDER.map((status)=>{
        ...
        const label = info.label + " · " + t("Skills ") + skillCount + " · " + t("Groups ") + groupCount;
        return '<button class="status-filter ' + info.cls + (active ? " active" : "") + '" data-action="status-filter" data-status="' + esc(status) + '" aria-pressed="' + (active ? "true" : "false") + '" title="' + esc(label) + '">' +
          ...
          '<span class="status-filter-counts"><span class="status-count" aria-label="' + esc(t("Skills")) + '">' + esc(skillCount) + '</span><span class="status-count-label">/</span><span class="status-count" aria-label="' + esc(t("Groups")) + '">' + esc(groupCount) + '</span></span>' +
          '</button>';
      }).join("");
    }
    function renderResultMeta(buckets){
      ...
      document.getElementById("resultMeta").textContent =
        (q ? t("Search results") : t("Current view"))
        + " · "
        + activeInfo.label
        + " · "
        + t("Skills ")
    ...
    function renderModeSwitch(){
      ...
      root.innerHTML = [
        ["workspaceToCentral", t("Workspace → Central")],
        ["centralToWorkspace", t("Central → Workspace")]
    ...
    function renderTabs(){
      ...
      root.innerHTML = ["all", ...tools].map((tool)=>{
        ...
        const label = tool === "all" ? t("All") : tool;
    ...
    function postMove(status, skills){
      ...
      if (targets.length === 0) { setStatus(t("There are no skills to apply."), "warn"); return; }
      vscode.postMessage({ type: "moveCompared", payload: { mode: uiState.mode, status, targets } });
      setStatus(t("Apply review requested: ") + statusInfoFor(status).label + " " + t("skills ") + targets.length + " · " + directionText(), "info");
    }
    function postMoveGroups(status, groups){
      if (uiState.busy) return;
      const groupIds = groupIdsForAction(status, groups);
      if (groupIds.length === 0) { setStatus(t("There are no groups to apply."), "warn"); return; }
      vscode.postMessage({ type: "moveComparedGroups", payload: { mode: uiState.mode, status, groupIds } });
      setStatus(t("Matching groups apply requested: ") + statusInfoFor(status).label + " " + t("groups ") + groupIds.length + " · " + directionText(), "info");
    }
    ...
    function renderSkillRow(skill, status){
      ...
      const delta = status === "modified"
        ? t("Changed {0} · Source only {1} · Target only {2}", skill.modifiedFileCount, sourceOnly, targetOnly)
        : status === "added"
          ? t("Files to add {0}", sourceFileCount(skill))
          : status === "removed"
            ? t("Files to delete {0}", targetFileCount(skill))
            : t("Same files {0}", sourceFileCount(skill));
      const actionLabel = status === "removed" ? t("Review Delete") : status === "added" ? t("Add") : t("Apply Change");
      const chips = groupChips(skill);
      const rowActions = '<span class="badge ' + info.cls + '">' + info.label + '</span>' +
        (isSame ? "" : '<button class="ghost" data-action="open-diff" data-key="' + esc(skill.key) + '" ' + (uiState.busy ? "disabled" : "") + '>' + esc(t("Diff")) + '</button>' +
      ...
      return '<div class="row ' + (uiState.busy ? "disabled" : "") + '">' +
        ...
        '<div class="meta">' + esc(sourceLabel()) + " " + esc(t("files ")) + esc(sourceFileCount(skill)) + " · " + esc(targetLabel()) + " " + esc(t("files ")) + esc(targetFileCount(skill)) + " · " + esc(delta) + '</div>' +
    ...
    function renderGroupRow(group, status){
      ...
      const actionLabel = status === "removed" ? t("Delete Group") : status === "added" ? t("Add Group") : t("Apply Group");
      ...
      return '<div class="row ' + (uiState.busy ? "disabled" : "") + '">' +
        ...
        '<div class="meta">' + esc(sourceLabel()) + " " + esc(t("targets ")) + esc(sourceCount) + " · " + esc(targetLabel()) + " " + esc(t("targets ")) + esc(targetCount) + (preview ? " · " + esc(preview + more) : "") + '</div></div>' +
        '<div class="row-actions">' + rowActions + '</div></div>';
    }
    function renderAgentBlock(status, tool, skills){
      const button = status === "same" || skills.length === 0 ? "" : '<button class="' + (status === "removed" ? "danger" : "primary") + '" data-action="move-agent" data-status="' + esc(status) + '" data-tool="' + esc(tool) + '" ' + (uiState.busy ? "disabled" : "") + '>' + esc(t("Apply This Agent")) + '</button>';
      return '<div class="agent-block"><div class="agent-head"><div><span class="agent-name">' + esc(tool) + '</span> <span class="agent-count">' + esc(t("Skills ")) + esc(skills.length) + '</span></div>' + button + '</div><div class="rows">' + skills.map((skill)=>renderSkillRow(skill, status)).join("") + '</div></div>';
    }
    function renderGroupAgentBlock(status, tool, groups){
      const button = status === "same" || groups.length === 0 ? "" : '<button class="' + (status === "removed" ? "danger" : "primary") + '" data-action="move-agent-groups" data-status="' + esc(status) + '" data-tool="' + esc(tool) + '" ' + (uiState.busy ? "disabled" : "") + '>' + esc(t("Apply This Agent Visible Groups")) + '</button>';
      return '<div class="agent-block"><div class="agent-head"><div><span class="agent-name">' + esc(tool) + '</span> <span class="agent-count">' + esc(t("Groups ")) + esc(groups.length) + '</span></div>' + button + '</div><div class="rows">' + groups.map((group)=>renderGroupRow(group, status)).join("") + '</div></div>';
    }
    function renderStatusSection(status, bucket){
      ...
      const currentPageSuffix = totalRows > PAGE_SIZE
        ? t(" · {0}/{1} pages", currentPage, pageCount(totalRows))
        : "";
      ...
      if (status !== "same" && skills.length > 0) {
        actions.push('<button class="' + (status === "removed" ? "danger" : "primary") + '" data-action="move-status" data-status="' + esc(status) + '" ' + (uiState.busy ? "disabled" : "") + '>' + esc(t("Apply All Skills")) + '</button>');
      }
      if (status !== "same" && groupDiffs.length > 0) {
        actions.push('<button class="' + (status === "removed" ? "danger" : "primary") + '" data-action="move-status-groups" data-status="' + esc(status) + '" ' + (uiState.busy ? "disabled" : "") + '>' + esc(t("Apply Visible Groups")) + '</button>');
      }
      ...
      const skillBody = visibleSkillRows.length
        ? '<div class="subsection"><div class="subsection-head"><span class="subsection-name">' + esc(t("Skills")) + '</span><span>' + esc(t("Skills ")) + esc(skills.length) + esc(currentPageSuffix) + '</span></div><div class="subsection-grid">' + skillBlocks + '</div></div>'
        : "";
      const groupBody = visibleGroupRows.length
        ? '<div class="subsection"><div class="subsection-head"><span class="subsection-name">' + esc(t("Skill Groups")) + '</span><span>' + esc(t("Groups ")) + esc(groupDiffs.length) + esc(currentPageSuffix) + '</span></div><div class="subsection-grid">' + groupBlocks + '</div></div>'
        : "";
      const empty = !skillBody && !groupBody ? '<div class="empty">' + esc(t("No skills or groups match the current filter.")) + '</div>' : "";
      const pager = renderPager(status, totalRows);
      const body = skillBody + groupBody + empty + pager;
      return '<section class="section"><div class="section-head"><div class="section-title"><span class="badge ' + info.cls + '">' + info.label + '</span><div><div class="section-name">' + esc(info.title) + ' · ' + esc(t("Skills ")) + esc(skills.length) + ' · ' + esc(t("Groups ")) + esc(groupDiffs.length) + '</div><div class="section-desc">' + esc(info.desc) + '</div></div></div><div class="section-actions">' + action + '</div></div>' + (body ? '<div class="section-body">' + body + '</div>' : '') + '</section>';
    }
    function render(){
      if (renderFrame) {
    ...
    function bindActions(){
      document.body.addEventListener("click",(ev)=>{
        ...
        if (action === "mode") {
          ...
          resetPages();
          setStatus(t("Direction changed: ") + directionText(), "info");
          render();
          return;
        }
        if (action === "tab") {
    ...
    window.addEventListener("message", (event)=>{
      ...
      if (msg.type === "ui") {
        ...
        setStatus(payload.message || (uiState.busy ? t("Working...") : t("Ready")), payload.tone || "info");
        render();
      }
    });
    ...
    setStatus(t("Ready") + " · " + directionText(), "info");
    bindActions();
    render();
  </script>
</body>
