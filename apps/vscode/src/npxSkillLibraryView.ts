  ...
  const openNpxSkillLibrary = async (): Promise<void> => {
    try {
      ...
      const render = (): void => {
        ...
        panel.webview.html = renderNpxSkillLibraryHtml(buildRows(args, side), side, args.getUiLanguage());
      };
...
function renderNpxSkillLibraryHtml(rows: NpxRepoRow[], side: TreeSide, language: UiLanguage): string {
...
function renderDetail(row: NpxRepoRow, active: boolean, t: (en: string, ko: string) => string): string {
  const canUpdate = row.repoUrl && row.skills.length > 0;
