...
function renderNpxSkillLibraryHtml(rows: NpxRepoRow[], side: TreeSide, language: UiLanguage, diagnosis: NpxSkillLibraryDiagnosis | null): string {
  ...
  </style></head><body><div class="wrap"><div class="top"><h1>${esc(t("NPX Skill Library", "NPX 스킬 라이브러리"))}</h1><div class="summary">${rows.length} ${esc(t("repo groups", "repo 그룹"))} · ${totalSkills} ${esc(t("skills", "스킬"))}</div></div>${renderEnvironmentBar(diagnosis, t)}<div class="tabs"><button class="tab ${side === "workspace" ? "active" : ""}" data-side="workspace">Workspace</button><button class="tab ${side === "central" ? "active" : ""}" data-side="central">Central</button><button class="primary" data-download="${side}" ${envReady ? "" : "disabled"}>${esc(t("Install", "설치"))}</button><button data-update-all="${side}" ${envReady ? "" : "disabled"}>${esc(t("Update all", "전체 업데이트"))}</button><button data-check-env>${esc(t("Check environment", "환경 점검"))}</button><button data-write-guide>${esc(t("Create guide", "가이드 생성"))}</button></div><main class="content"><section class="panel"><table><thead><tr><th>${esc(t("Repo", "Repo"))}</th><th style="width:82px">${esc(t("Agent", "에이전트"))}</th><th style="width:70px">${esc(t("Skills", "스킬"))}</th><th style="width:150px">${esc(t("Updated", "업데이트"))}</th></tr></thead><tbody>${rows.map((row, index) => renderRow(row, index === 0)).join("") || `<tr><td colspan="4"><div class="empty">${esc(t("No npx groups on this side.", "이 위치에는 npx 그룹이 없습니다."))}</div></td></tr>`}</tbody></table></section><section class="panel">${rows.map((row, index) => renderDetail(row, index === 0, t, envReady)).join("") || `<div class="empty">${esc(t("Use Install to add skills from npx.", "설치 버튼으로 npx 스킬을 설치하세요."))}</div>`}</section></main></div><script nonce="${nonce}">
